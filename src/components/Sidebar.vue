<template>
  ...
  >
    <div class="flex flex-col h-full py-4">
      <!-- User Profile Card -->
      <div class="px-4 mb-4">
        <div class="bg-gradient-to-br from-purple-500 to-indigo-600 rounded-xl p-4 text-white shadow-md">
          <div class="flex items-center gap-3 mb-3">
            <div class="w-10 h-10 rounded-full bg-white/20 backdrop-blur-sm flex items-center justify-center ring-2 ring-white/50">
          ...
          <div class="flex items-center justify-between text-xs bg-white/10 rounded-lg px-3 py-2 backdrop-blur-sm">
          ...
          <div class="bg-green-50 dark:bg-green-900/20 rounded-lg p-2 border border-green-200 dark:border-green-800">
            <div class="flex items-center gap-1.5">
              ...
              <div>
                ...
                <p v-if="loading" class="text-xs font-bold text-green-700 dark:text-green-300 animate-pulse">...</p>
                <p v-else class="text-xs font-bold text-green-700 dark:text-green-300">{{ formattedIncome }}</p>
              </div>
            </div>
          </div>
          <div class="bg-red-50 dark:bg-red-900/20 rounded-lg p-2 border border-red-200 dark:border-red-800">
            <div class="flex items-center gap-1.5">
              ...
              <div>
                ...
                <p v-if="loading" class="text-xs font-bold text-red-700 dark:text-red-300 animate-pulse">...</p>
                <p v-else class="text-xs font-bold text-red-700 dark:text-red-300">{{ formattedExpense }}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <!-- Navigation Label -->
      <div class="px-4 mb-2">
        <h3 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Navegación</h3>
      </div>
      ...
      <nav class="flex-1 px-3 space-y-1 overflow-y-auto">
        ...
        >
          <div class="flex items-center justify-center w-8 h-8 rounded-lg transition-all"
               :class="isActive('/panel-control')
                 ? 'bg-white/20'
                 : 'bg-purple-100 dark:bg-purple-900/30 group-hover:scale-110'">
            <svg class="w-4 h-4" :class="isActive('/panel-control') ? 'text-white' : 'text-purple-600 dark:text-purple-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" />
            </svg>
          </div>
          <span class="font-medium text-sm">Panel de Control</span>
          <div v-if="isActive('/panel-control')" class="absolute right-0 w-1 h-8 bg-white rounded-l-full"></div>
        </RouterLink>

        <RouterLink
          to="/ingresos"
          class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group relative overflow-hidden"
          :class="isActive('/ingresos')
        ...
        >
          <div class="flex items-center justify-center w-8 h-8 rounded-lg transition-all"
            ...
            <svg class="w-4 h-4" :class="isActive('/ingresos') ? 'text-white' : 'text-green-600 dark:text-green-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 11l5-5m0 0l5 5m-5-5v12" />
            </svg>
          </div>
          <span class="font-medium text-sm">Ingresos</span>
          <div v-if="isActive('/ingresos')" class="absolute right-0 w-1 h-8 bg-white rounded-l-full"></div>
        </RouterLink>

        <RouterLink
          to="/egresos"
          class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group relative overflow-hidden"
          :class="isActive('/egresos')
        ...
        >
          <div class="flex items-center justify-center w-8 h-8 rounded-lg transition-all"
            ...
            <svg class="w-4 h-4" :class="isActive('/egresos') ? 'text-white' : 'text-red-600 dark:text-red-400'" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 13l-5 5m0 0l-5-5m5 5V6" />
            </svg>
          </div>
          <span class="font-medium text-sm">Egresos</span>
          <div v-if="isActive('/egresos')" class="absolute right-0 w-1 h-8 bg-white rounded-l-full"></div>
        </RouterLink>

        <!-- Divider -->
        <div class="h-px bg-gradient-to-r from-transparent via-gray-300 dark:via-gray-600 to-transparent my-3"></div>

        <!-- Additional Links -->
        <button class="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700/50 hover:text-purple-600 dark:hover:text-purple-400 w-full">
          <div class="flex items-center justify-center w-8 h-8 rounded-lg bg-gray-100 dark:bg-gray-700/50 group-hover:scale-110 transition-all">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" />
            </svg>
          </div>
          <span class="font-medium text-sm">Reportes</span>
          <span class="ml-auto text-xs bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 px-2 py-0.5 rounded-full font-semibold">Próximamente</span>
        </button>
      </nav>
      ...
      <div class="px-4 pt-3 border-t border-gray-200 dark:border-gray-700 space-y-2">
        <!-- Help Button -->
        <button class="flex items-center gap-2 w-full px-3 py-2 rounded-lg text-xs text-gray-600 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-700/50 transition-all group">
        ...
        <!-- Version Info -->
        <div class="flex items-center justify-between px-3 py-2 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-lg">
          <div class="flex items-center gap-2">
            <div class="w-6 h-6 rounded bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center">
...
const loadBalance = async () => {
  ...
  try {
    ...
    console.log('Sidebar - Balance cargado:', { balance: balance.value, income: totalIncome.value, expense: totalExpense.value });
  } catch (error) {
